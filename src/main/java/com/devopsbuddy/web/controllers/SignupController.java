...
public class SignupController {
	...
	public String signupGet(@RequestParam("planId") final int planId, final ModelMap model) {
	...
	public String signUpPost(@RequestParam(name = "planId", required = true) final int planId,
			@RequestParam(name = "file", required = false) final MultipartFile file,
			@ModelAttribute(PAYLOAD_MODEL_KEY_NAME) @Valid final ProAccountPayload payload,
		...
		final User user = UserUtils.fromWebUserToDomainUser(payload);

		// Stores the profile image on Amazon S3 and stores the URL in the user's record
		if (file != null && !file.isEmpty()) {
			final String profileImageUrl = s3Service.storeProfileImage(file, payload.getUsername());
		...
		final Plan selectedPlan = planService.findPlanById(planId);
		...
		if (planId == PlansEnum.BASIC.getId()) {
			...
			final Map<String, Object> stripeTokenParams = StripeUtils.extractTokenParamsFromSignupPayload(payload);
			...
			final String stripeCustomerId = stripeService.createCustomer(stripeTokenParams, customerParams);
	...
	public ModelAndView signupException(final HttpServletRequest request, final Exception exception) {
	 ...
	 **/
	private void checkForDuplicates(final BasicAccountPayload payload, final ModelMap model) {
